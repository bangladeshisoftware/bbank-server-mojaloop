...
function parseUserAgent(ua = '') {
  ...
  // Extract version
  const verMatch = ua.match(/(Chrome|Firefox|Safari|Edg|OPR|Edge)\/([\d.]+)/);
...
exports.getStats = async (req, res) => {
  try {
    ...
    const scopeWhere =
      userRole === 'MERCHANT' ? `WHERE merchant_id = '${userMerchantId}'` : '';

    const scopeAnd =
      userRole === 'MERCHANT' ? `AND merchant_id = '${userMerchantId}'` : '';
